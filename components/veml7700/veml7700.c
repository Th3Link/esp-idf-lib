...
esp_err_t veml7700_ambient_light(i2c_dev_t *dev, veml7700_config_t *config, uint32_t *value)
{
    uint16_t raw_value = 0;
    CHECK(read_port(dev, VEML7700_COMMAND_CODE_ALS, &raw_value));
    
    *value = (raw_value * resolution(config)) / 1000;
    return ESP_OK;
}

esp_err_t veml7700_white_channel(i2c_dev_t *dev, veml7700_config_t *config, uint32_t *value)
{
    uint16_t raw_value = 0;
...
{
    uint16_t raw_value = 0;
    CHECK(read_port(dev, VEML7700_COMMAND_CODE_ALS_INT, &raw_value));
    *high_threshold = raw_value & VEML7700_INTERRUPT_STATUS_HIGH_MASK;
    *low_threshold = raw_value & VEML7700_INTERRUPT_STATUS_LOW_MASK;
    return ESP_OK;
}
